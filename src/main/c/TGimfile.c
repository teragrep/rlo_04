 ...
 * Copyright 2021 Suomen Kanuuna Oy.
 *
...
// include TG implementation of readMultiLine
// TODO bit dirty but until further re-factoring of imfile.c is done, we'll keep it separate
#include "readFile.h"
...
pollFileReal(lstn_t *pLstn, cstr_t **pCStr)
{
	int64 strtOffs;
	DEFiRet;
	...
	while(glbl.GetGlobalInputTermState() == 0) {
		...
		if(pLstn->startRegex == NULL) {
			if (pLstn->pStrm->pszFName != NULL) {
				CHKiRet(isAllowedSuffix(pLstn->pStrm->pszFName, pLstn->pStrm->prevMsgSegment, 0));
			}
			...
			CHKiRet(readMultiLine(pLstn->pStrm, pCStr, &pLstn->end_preg,
...
static rsRetVal ATTR_NONNULL(1)
pollFile(lstn_t *pLstn)
{
	cstr_t *pCStr = NULL;
	DEFiRet;

	int oldCancelState = 0;
	// entering critical section, all reads are committed within
	if (pthread_setcancelstate(PTHREAD_CANCEL_DISABLE, &oldCancelState) == 0)
	{
		iRet = pollFileReal(pLstn, &pCStr);
	}
	else {
		ABORT_FINALIZE(RS_RET_ERR);
	}
	// force thread cancellation back on
	while (pthread_setcancelstate(oldCancelState, &oldCancelState) != 0)
		continue;
...
doPolling(void)
{
	...
	while(glbl.GetGlobalInputTermState() == 0) {
		...
		for(pLstn = runModConf->pRootLstn ; pLstn != NULL ; pLstn = pLstn->next) {
			...
			while (pollFile(pLstn) == RS_RET_OK) {
...
in_removeFile(const int dirIdx, lstn_t *const __restrict__ pLstn, const sbool bRemoveStateFile)
{
	...
	pollFile(pLstn); /* one final try to gather data */
...
do_inotify(void)
{
	...
	int rd;
	int currev;
	static time_t timeoutEnforcer = 0;
...
dbgprintf("pre inotify_init\n");
	// force waiting on the poll rather than on the read this IN_NONBLOCK
...
dbgprintf("post setuInitialWatches\n");
	...
	while(glbl.GetGlobalInputTermState() == 0) {
		...
		if(r == 0) {
			...
			if(runModConf->haveReadTimeouts) {
				time_t now = getTime(NULL);
				if (timeoutEnforcer + (runModConf->timeoutGranularity / 1000) < now) {
					in_do_timeout_processing();
					timeoutEnforcer = now;
				}
			}
...
CODESTARTwillRun
	...
	CHKiRet(prop.SetString(pInputName, UCHAR_CONSTANT("TGimfile"), sizeof("TGimfile") - 1));
